...
**Watch the 65% override.** 7.8 gph is about 0.56 gph *below* the 8.4 gph the default view plans at the same 65% setting — so an override left sitting on 65% shows you **more** fuel remaining than the default view does, roughly 1.7 gal over a three-hour cruise. That is the optimistic direction. Use the override to compare power settings, then clear it (keep tapping past 75%) before you fly the numbers.
