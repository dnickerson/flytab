...
- **A route FlyTab planned for you — 8.1 gph at 65%.** This is the normal case. The route table looks up the measured power table in the aircraft configuration and uses the recorded band closest to the power percentage in effect; 65% is the default, and that band reads 8.1. **That 8.1 is the band median, 0.3 gph below the 8.4 planning figure above** — so a normally planned route currently shows about **0.9 gal more remaining over a three-hour cruise** than the 8.4 figure would. Measured burn at other recorded settings on this aircraft ranges from 5.0 gph at 42% power up to 8.9 gph at 75%.
- **Rows FlyTab had to build itself — 9.0 gph.** When a leg arrives with no climb/cruise/descent breakdown — a waypoint you added by hand, or a plan imported without segment detail — the route table generates its own segments using the `cruise_gph` figure from the aircraft configuration, which is 9.0. That is deliberately higher than both 8.1 and 8.4: it over-plans the burn, which is the safe side. (A plan imported *with* segment detail keeps whatever burn rates that plan was built with — those are not FlyTab's numbers and FlyTab does not second-guess them.)
...
**Watch the 65% override.** 7.8 gph is about 0.26 gph *below* the 8.1 gph that the same 65% setting gives from the measured table — so an override left sitting on 65% shows you slightly **more** fuel remaining than the default view does, roughly 0.8 gal over a three-hour cruise, and about 1.8 gal more than the 8.4 planning figure. That is the optimistic direction. Use the override to compare power settings, then clear it (keep tapping past 75%) before you fly the numbers.
