...
**Cruise burn is not a single fixed number — it depends on which cruise power you're flying.** FlyTab looks up the measured power table in the aircraft configuration and uses the recorded band closest to the power percentage in effect: 8.1 gph is what you'll see at the aircraft's normal 65% power, which is the default. Measured burn at other recorded settings on this aircraft ranges from 5.0 gph at 42% power up to 8.9 gph at 75%. Tapping the route table's **%PWR** column header cycles a cruise-power override; while one is active, FUEL, REM, TOTAL, and DEST all recompute for that power setting instead of the default 65% figure — don't be surprised to see a number other than 8.1 gph if a cruise-power override has been set. One caveat when comparing to the engine page: the **% power label** on those bands is calculated from RPM and manifold pressure, the way the Lycoming charts do it, while the engine monitor computes power from fuel flow when you are running lean of peak. The **gallons-per-hour figure is the measured ground truth** in both places; the percentage beside it is a label for the band, not a number to compare across the two screens.
