...
Planned fuel figures — the route table's FUEL and REM columns, the TOTAL footer, and the DEST badge — are built from three burn rates: **climb 15.0 gph, cruise 8.1 gph at the aircraft's normal 65% power, descent 6.9 gph.** Climb and descent are those figures in every case. **Cruise is not** — depending on how the rows were produced and whether you have set a cruise-power override, the number behind the cruise rows can be 8.1, 9.0, or 7.8 gph. Which one applies is spelled out under *Which cruise number is actually in use* below, and it is worth knowing, because one of the three reads slightly optimistic.

These are **measured, not book numbers.** Climb and descent are each the **85th-percentile** fuel flow recorded by the engine monitor for that phase, across 53 logged flights in N194JT (climb p85 is 15.1 gph, carried as 15.0; descent p85 is 6.9) — deliberately the high side of normal rather than the average, because an under-estimate quietly over-states the fuel you will have left, and that is the error that runs tanks dry. Cruise is measured a different way: instead of a percentile pooled across every cruise sample regardless of power setting, 8.1 gph is the **measured burn at the aircraft's actual configured cruise power, 65%,** drawn from 1,120 logged samples flown at that setting specifically. (Pooling every cruise sample regardless of power gives a p85 of 8.3 gph — 0.2 gph higher, which is inside this carbureted engine's normal fuel-flow scatter of roughly half a gallon an hour, so it is not treated as a separate, more conservative figure.)
...
- **A route FlyTab planned for you — 8.1 gph at 65%.** This is the normal case. The planner looks up the measured power table in the aircraft configuration and uses the recorded band closest to the power percentage in effect; 65% is the default. Measured burn at other recorded settings on this aircraft ranges from 5.0 gph at 42% power up to 8.9 gph at 75%.
- **Rows FlyTab had to build itself — 9.0 gph.** When a leg arrives with no climb/cruise/descent breakdown — a waypoint you added by hand, or a plan imported without segment detail — the route table generates its own segments using the `cruise_gph` figure from the aircraft configuration, which is 9.0. That is deliberately higher than 8.1: it over-plans the burn, which is the safe side. (A plan imported *with* segment detail keeps whatever burn rates that plan was built with — those are not FlyTab's numbers and FlyTab does not second-guess them.)
- **A cruise-power override you have set — 7.8 gph at 65%.** Tapping the route table's **%PWR** column header cycles an override through 55%, 65%, 75%, then off again. While one is active, FUEL, REM, TOTAL and DEST recompute the cruise rows from power and specific fuel consumption (% power × 180 hp × 0.067) rather than from the measured band table: **6.6 gph at 55%, 7.8 at 65%, 9.0 at 75%.**

**Watch the 65% override.** 7.8 gph is about 0.26 gph *below* the 8.1 gph that the same 65% setting gives from the measured table — so an override left sitting on 65% shows you slightly **more** fuel remaining than the default view does, roughly 0.8 gal over a three-hour cruise. That is the optimistic direction. Use the override to compare power settings, then clear it (keep tapping past 75%) before you fly the numbers.
