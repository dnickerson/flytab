...
export function decomposeLeg(profile, leg) {
    ...
    if (leg.endingAtGround) {
        ...
        //   - profile WITHOUT it:     fuel_burn_gph * (0.55/0.75)       = 5.94 gph
        // 5.94 is below the RV-9A's measured descent p85 of 6.9 gph, so that
        // branch under-plans descent burn and over-states fuel remaining — the
        // direction that runs tanks dry. Measured descent burn here is 85% of
        // cruise (6.9 of 8.1), so "descent costs nothing" is simply wrong for
        // this airframe; absent real data, assume no fuel saving in the descent.
        // Math.max, not a replacement: the 9.21 branch is already conservative
        // and must not be lowered to 8.1 by this change.
